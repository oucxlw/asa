...
class Trainer(object):
    ...
    def run(self, train_loader, dev_loader, num_epochs=50, visdom=1, visdom_id='Loss(clean)'):
        if visdom:
            from visdom import Visdom
            vis = Visdom(env=visdom_id)
            vis_opts = dict(title=visdom_id,
                   ylabel='Loss', xlabel='Epoch',
                   legend=['train loss', 'cv loss'])
            vis_window=None 
            vis_epochs = th.arange(1, num_epochs + 1)
            tr_loss = th.Tensor(num_epochs)
            cv_loss = th.Tensor(num_epochs)
        ...
        with th.cuda.device(self.gpuid[0]):
            ...
            while self.cur_epoch < num_epochs:
                ...
                # visualization
                if visdom:
                    tr_loss[self.cur_epoch] = tr["loss"]
                    cv_loss[self.cur_epoch] = cv["loss"]
                    x_axis = vis_epochs[0:self.cur_epoch + 1]
                    y_axis = th.stack(
                            (tr_loss[0:self.cur_epoch + 1], cv_loss[0:self.cur_epoch + 1]), dim=1
                            )
...
class SiSnrTrainer(Trainer):
    ...
    def compute_loss(self, egs):
        ...
        ests, spk_pred = self.nnet(egs['mix1'], egs['mix2'], egs['aux'], egs['aux_len'])
