...
def attention(mix, aux):    
    T = mix.shape[-1]
    aux = th.unsqueeze(aux, -1)
    aux = aux.repeat(1,1,T)
    att = th.sum(mix*aux, 1, keepdim=True) 
    att = F.softmax(att, -1)
    att = att * aux
    return att + aux 
...
class ConvTasNet(nn.Module):
    def __init__(self,
        ...
        self.average = Conv1D(N, N, L, L, groups=N)
        self.average.weight = nn.Parameter(th.ones(N, 1, L) / L)
        self.average.bias = nn.Parameter(th.zeros(N))
        ...
        self.pred_linear = nn.Linear(N,101)
    ...
    def forward(self, mix, aux, aux_len):
        # n x 1 x S => n x N x T
        if mix.dim() == 1:
        ...
        upsampling = nn.Upsample(size=y.shape[-1], mode='nearest')
        
        y_ave = self.average(y) 
        att_out = attention(y_ave, aux) 
